...
tombs = [16,17,18,20,322,396,677,711,713,720,722,772,378,421,449,450,452,459,500,501,638,642,652,653,681,697,765,790]
...
def loadData():
    ...
    global OSRDATA
    global OSRfd
    OSRfd = open("./yltmp/object-spatial-relation-dataset.txt", 'r')#open("./yltmp/OSRdataset.txt", 'r')#
...
def deletePriors(idxes):
    ...
    for i in range(lenData):
        ...
        j = int(thisPrior.split(';')[-1][:-1])
        lenJ = len(thisPrior) - len(thisPrior.split(';')[-1])
        if j != i:
            print("uncontinous error %d"%(i))
            break
        if( cnt < len(idxes)) and (j == idxes[cnt]):
            cnt += 1
            continue
        bufferFd.write("%s %d\n"%(thisPrior[:lenJ], i - cnt))
    bufferFd.close()
    dataFd.close()
...
def uploadDataFromMethods():
    ...
    for i in range(lenLocal):
        hotPrior = hotLines[i]
        lenThisHot = len(hotPrior) - 1 #maybe "\n" at last
        localPrior = localLines[i]
        if not (localPrior[:lenThisHot] == hotPrior[:-1]):
            diffFlag = True
            print(i)
...
def appendDataFromMethods():
    hotDatafd = open("./layoutmethods/object-spatial-relation-dataset.txt", 'r')
    localDatafd = open("./yltmp/object-spatial-relation-dataset.txt", 'r')
    hotLines = hotDatafd.readlines()
    localLines = localDatafd.readlines()
    lenLocal = len(localLines)
    lastPrior = localLines[lenLocal-1]

    a = len(lastPrior.split(';')[-1])+1
    idx = int(lastPrior.split(';')[-1][:-1])
    lastPrior = lastPrior[:(len(lastPrior) - a)]
    
    lenHot = len(hotLines)
    diffFlag = -1
    sign = -1
    for i in range(lenHot):
        hotPrior = hotLines[i]
        if (lastPrior == hotPrior[:-1]):
            diffFlag = i
            sign = idx
            print(idx)
    ...
    localDatafd.close()
    localDatafd = open("./yltmp/object-spatial-relation-dataset.txt", 'a')
    for i in range(diffFlag+1, lenHot):
        sign += 1
        localDatafd.write("%s; %d\n"%(hotLines[i][:-1], sign))
    localDatafd.close()
    hotDatafd.close()
...
if __name__ == '__main__':
    #uploadDataFromMethods()
    #appendDataFromMethods()
    #deletePriors([547,548,605,606,607,608,609,610,611,612,613,614,649,650])
    loadData()
    #for i in OSRDATA:
        #for j in i:
            #print(j, i[j])
    ...
    c = 0
    for osr in OSRDATA:
        if osr["userName"] == "luojia":#"lez":  #
            cnt += 1
            if ("gtrans" in osr):
                c += len(osr["gtrans"])
    print("%d, %d, %f"%(cnt, c+cnt, 0.5*cnt + 0.25*(c+cnt)))
