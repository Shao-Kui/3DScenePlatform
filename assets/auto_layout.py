...
def exec_transfer(geo,rtype,roomid):
	...
	geopoints_template=np.array(reduce(lambda x,y:x+y,getroomcontour(origin,roomname)))
	bbox_template={"min":geopoints.min(axis=0),"max":geopoints.max(axis=0)}	
	...
	for obj in room_template["objList"]:
		...
		translate=np.array([[cos(rotate/180*np.pi),0,sin(rotate/180*np.pi)],[0,1,0],[-sin(rotate/180*np.pi),0,cos(rotate/180*np.pi)]]).dot(np.array(obj["translate"])-[center_template[0],0,center_template[1]])+[center[0],0,center[1]]
...
def generate():
	#data={"roomtype":"Bedroom","origin":"000514ade3bcc292a613a4c2755a5050","level":0,"roomid":"fr_0rm_0"}
	contour_to_transfer=getroomcontour(data["origin"],data["roomid"])
