    ...
    for i in range(length, -1, -1):
        for sta in range(1 << length):
            if cnt[sta] == i and f[sta]:
                success = action_2(mods,
                                   space,
                                   subject_to=f_subject_to[sta],
                                   target_attributes=f_target_attributes[sta])
                if success:
...
def get_path(sforder: int, poly1: Polygon, poly2: Polygon, info: TransformInfo, p1: np.ndarray, p2: np.ndarray,
    ...
    if hack_bound:
        poly_new1=scale(poly1,1.1,2)
        poly_new2=scale(poly2,1.1,2)
        poly_new=poly_new1.union(poly_new2)
        remov_indexes=[]
        for index in reachable_index3:
            x, y = x_web[index[0]], y_web[index[1]]
            if not intersect_check(translate(poly_new, x, y), space, obstacles, False):
                remov_indexes.append(index)
        for index in remov_indexes:
            reachable_index3.remove(index)
...
def output_scene(mods: List[TransformableModule], scene: json, idx: int, scene_name: str, target_attributes: list):
    ...
    with open('outputs/{}/{}.json'.format(scene_name, idx), 'w') as outf:
        json.dump(out, outf)
...
def wall_relative_2(nearest_dis: float, second_dis: float, nearest_rot: float, poly_bounds: tuple, scalex: float,
                    scalez: float, space: TwoDimSpace, force_all: bool):
    wall_count = len(space.walls)
    # rand_num = random.randint(0, wall_count - 1)
    global rand_num 
...
def action_2(mods: List[TransformableModule], space: TwoDimSpace, subject_to: List[int], target_attributes: List[int]):
    ...
    for idx in top_idxes:
        ...
        for relation in gtrans:
            for i in range(length):
                target = mods[i]
                if i != idx and not completed[i] and target.name == relation['attachedObjId'] and target.state_list[
                        target.target_state].name == relation['currentState']:
        ...
        for k in range(100):
            ...
            if len(wall_rel) > 0:
                ...
                main_p, main_rot = wall_relative_2(nearest_dis, second_dis, nearest_rot, poly_bounds, scalex, scalez,
                                                   space, k < 50)
...
def main():
    ...
    if os.path.exists('outputs/{}'.format(name)):
        for file in os.listdir('outputs/{}'.format(name)):
            os.remove('outputs/{}/{}'.format(name, file))
    else:
    ...
    if os.path.exists('outputs/{}_scenes'.format(name)):
        ...
        os.mkdir('outputs/{}_scenes'.format(name))
